...
(function () {
  ...
  // One end of a numeric range, or null for "off" — the same rule as _lim() on the server, including the
  // courtesy that a probability typed as a whole percentage (62) means 0.62. Edge is passed pct=false
  // because its column is in points and points genuinely do exceed 1.
  function lim(name, pct) {
    var v = parseFloat((val(name) || "").trim());
    if (isNaN(v)) return null;
    return (pct && v > 1) ? v / 100 : v;
  }

  // An absent attribute is a row with no value in that column, and it fails a range it cannot answer —
  // the same as the server dropping rows whose column is None. `undefined` parses to NaN, so the two
  // comparisons below are false and the row is hidden without needing a case of its own.
  function inBand(raw, lo, hi) {
  ...
  function visible(tr) {
    ...
    // A checkbox, so `checked` and not `value` — an unchecked box still has value "1". The class is
    // the server's own verdict on the row, the same one the highlight and the picks count read.
    var p = form.elements["picks"];
    if (p && p.checked && !tr.classList.contains("bet")) return false;
    ...
    if (!inBand(d.fair, lim("fair_min", true), lim("fair_max", true))) return false;
    if (!inBand(d.quote, lim("quote_min", true), lim("quote_max", true))) return false;
    if (!inBand(d.model, lim("model_min", true), lim("model_max", true))) return false;
    // No conversion: data-edge is already in points, rounded to the precision its cell prints at.
    if (!inBand(d.edge, lim("edge_min", false), lim("edge_max", false))) return false;
